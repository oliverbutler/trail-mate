...
const dbConnectionStringSecretVersion = new aws.secretsmanager.SecretVersion(
  'dbConnectionStringSecretVersion',
  {
...
const trailMateIgw = new aws.ec2.InternetGateway('trailMateIgw', {
  vpcId: trailMateVpc.id,
});
...
const trailMateSubnetAAssociation = new aws.ec2.RouteTableAssociation(
  'trailMateSubnetAAssociation',
  {
...
const trailMateSubnetBAssociation = new aws.ec2.RouteTableAssociation(
  'trailMateSubnetBAssociation',
  {
...
const bastionEip = new aws.ec2.Eip('bastionEip', {
  instance: tailMateBastion.id,
});

export const bastionIp = bastionEip.publicIp;

const trailMateRepository = new aws.ecr.Repository('trailMateRepository', {
...
const trailMateAlb = new aws.lb.LoadBalancer('trailMateAlb', {
  ...
  subnets: [trailMateSubnetA.id, trailMateSubnetB.id],
});
...
const ecsExecutionSecretsAccess = new aws.iam.RolePolicy(
  'ecsExecutionSecretsAccess',
  {
        ...
        Statement: [
          {
            ...
            Resource: [
              `arn:aws:secretsmanager:eu-west-2:${current.accountId}:secret:*`,
            ],
          },
        ],
      })
...
const ecsExecutionRolePolicyAttachment = new aws.iam.RolePolicyAttachment(
...
const trailMateLogs = new aws.cloudwatch.LogGroup('trailMateLogs', {
...
const ecsLoggingPolicy = new aws.iam.Policy('ecsLoggingPolicy', {
  ...
  policy: trailMateLogs.arn.apply((arn) =>
    JSON.stringify({
      version: '2012-10-17',
      statement: [
        {
...
const ecsExecutionLoggingPolicyAttachment = new aws.iam.RolePolicyAttachment(
  'ecsExecutionLoggingPolicyAttachment',
  {
    role: ecsExecutionRole.name,
...
const githubActionsEcsDeploy = new aws.iam.Policy('githubActionsEcsDeploy', {
  ...
  description:
    'Allows GitHub Actions to deploy to ECS and manage ECR repositories',
  policy: pulumi
    .all([
      trailMateRepository.arn,
      trailMateLogs.arn,
      ecsExecutionRole.arn,
      ecsTaskRole.arn,
      current,
      trailMateCluster.name,
      trailMateService.name,
    ])
    .apply(
      ([
        trailMateRepositoryArn,
        trailMateLogsArn,
        ecsExecutionRoleArn,
        ecsTaskRoleArn,
        current,
        trailMateClusterName,
        trailMateServiceName,
      ]) =>
        JSON.stringify({
          version: '2012-10-17',
          statement: [
            ...
            },
            {
              ...
              Effect: 'Allow',
              Action: ['iam:PassRole'],
              Resource: [ecsExecutionRoleArn, ecsTaskRoleArn],
            },
            {
              Sid: 'DeployService',
              Effect: 'Allow',
              ...
              Resource: [
                `arn:aws:ecs:eu-west-2:${current.accountId}:service/${trailMateClusterName}/${trailMateServiceName}`,
              ],
            },
          ],
        })
...
const githubActionsUser = new aws.iam.User('githubActionsUser', {
...
const trailMateDbSubnetGroup = new aws.rds.SubnetGroup(
  'trailMateDbSubnetGroup',
  {
    name: 'trail-mate-db-subnet-group',
    description: 'Managed by Terraform',
    subnetIds: [trailMateSubnetA.id, trailMateSubnetB.id],
  }
);
...
const trailMateZone = new aws.route53.Zone('trailMateZone', {
  name: rootDomainName,
});
